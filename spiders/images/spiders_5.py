...
def get_item(item, main_title):
    ...
    for item_link in item_links:
        threads.append(threading.Thread(target=get_images,
                       args=(title, item_link, main_title)))
        ...
        try:
            for img in img_list:
                ...
                with open(folder + img_link.split('/')[-1], 'wb') as f:
                    starttime = time.time()
                    image = requests.get(
                        url=img_link, headers=headers, timeout=30).content
                    f.write(image)
                    time.sleep(1)
                    endtime = time.time()
                    print(f'正在保存>>>{title}' + '>>>' + img_link.split('/')
                          [-1] + '>>>用时%.3f' % (endtime - starttime), 'seconds')
        except IndexError:
            pass
